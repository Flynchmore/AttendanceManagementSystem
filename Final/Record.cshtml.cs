...
namespace AttendanceSystem2.Pages
{
    public class RecordModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostDeleteAllAsync()
        {
            try
            {
                await _mongoDbService.DeleteAllAttendancesAsync();
                TempData["Message"] = "All attendance records have been deleted successfully.";
            }
            catch (Exception ex)
            {
                TempData["Error"] = "Failed to delete attendance records: " + ex.Message;
            }
